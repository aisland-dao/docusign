...
document.getElementById("docdownload").onclick = async () => {
  if (currentDocumentId > 0) {
    ...
    if (blob.length > 0) {
      ...
      let url =
        window.location.protocol + "//" + window.location.host + "/docdownload";
      url = url + `?${qs.stringify(params)}`;
      window.open(url);
    }
  }
...
document.getElementById("docrename").onclick = () => {
  if (currentDocumentId > 0) {
    ...
    let c =
      '<input type="text" size="32" maxlength="64" id="docrename" value="' +
      docdata.description +
      '">';
    c =
      c +
      ' <button type="button" class="btn btn-secondary" id="docwrite"><i class="bi bi-pencil-square"></i></button>';
...
document.getElementById("docedit").onclick = async () => {
  if (currentDocumentId > 0) {
    ...
    if (docdata.originalfilename.slice(-4) != ".dcs") return;
...
document.getElementById("docdelete").onclick = async () => {
  if (currentDocumentId > 0) {
    ...
    if (docdata.status != "draft") return;
    let text =
      'Do you confirm the cancellation of "' +
      docdata.description +
    ...
    if (confirm(text) == true) {
      ...
      let url =
        window.location.protocol + "//" + window.location.host + "/docdelete";
      const response = await fetch(url + `?${qs.stringify(params)}`, {
        method: "GET",
      });
      let answerJSON = await response.json();
...
document.getElementById("docsign").onclick = async () => {
  if (currentDocumentId > 0) {
    ...
    if (docdata.status != "draft" && docdata.status != "waiting") return;
    // hide the current modal
    actionsModal.hide();
    // set the data in the table
    document.getElementById("docsigndescription").innerHTML =
      docdata.description;
    document.getElementById("docsignfilename").innerHTML =
...
document.getElementById("docreject").onclick = async () => {
  if (currentDocumentId > 0) {
    let docdata = get_document_data(currentDocumentId);
    //we can delete only documents in draft
    if (docdata.status != "waiting") return;
    let text =
      'Do you confirm the rejection of "' +
      docdata.description +
      '"?\nThe operation is irreversible';
    if (confirm(text) == true) {
      const params = {
      ...
      let url =
        window.location.protocol + "//" + window.location.host + "/docreject";
      const response = await fetch(url + `?${qs.stringify(params)}`, {
        method: "GET",
      });
      let answerJSON = await response.json();
...
document.getElementById("doclink").onclick = async () => {
  if (currentDocumentId > 0) {
    ...
    let url =
      window.location.protocol + "//" + window.location.host + "/doclink";
    const response = await fetch(url + `?${qs.stringify(params)}`, {
      method: "GET",
    });
    let answerJSON = await response.json();
...
document.getElementById("docsignsign").onclick = async () => {
  ...
  if (currentDocumentId > 0) {
    ...
    msg =
      msg + `You are connected to ${chain} using ${nodeName} v${nodeVersion}`;
    msg = msg + "</center></div>";
    document.getElementById("docsignmsg").innerHTML = msg;
    ...
    let url =
      window.location.protocol + "//" + window.location.host + "/getprivatekey";
    const responsek = await fetch(url + `?${qs.stringify(params)}`, {
      method: "GET",
    });
    let answerJSONk = await responsek.json();
    let flagpk = true;
    if (
      answerJSONk.answer == "KO" ||
      typeof answerJSONk.encryptionkey === "undefined"
    ) {
      flagpk = false;
    ...
    if (flagpk) {
      if (answerJSONk.encryptionkey.length == 0) flagpk = false;
    }
    if (!flagpk) {
...
async function connectWallet(accountid, event) {
  ...
  if (allInjected.length == 0) {
    ...
    msg =
      msg +
      'There is no wallet extension in your browser. Please install the <a href="https://polkadot.js.org/extension/" target="_blank">Polkadot Wallet Extension</a>';
    msg =
      msg +
      ' or <a href="https://www.subwallet.app" target="_blank">Subwallet</a>.';
  ...
  if (allAccounts.length == 0) {
    ...
    msg =
      msg +
      "There are no accounts in your wallet. Please create one and click the button again when done";
    msg = msg + "</center></div>";
    document.getElementById("msg").innerHTML = msg;
    return;
  }
...
async function render_main(section) {
  ...
  if (signaturetokenfirstview == signaturetoken && signaturetoken.length > 0) {
    ...
    document.cookie =
      "signaturetokenfirstview=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
  }
  // title
  let c = '<div class="row">';
  c =
    c +
    '<div class="col-8 pb-1" style="background-color:#D2E3FF" ><center><h2>Blockchain Documents Signing</h2></center></div>';
  ...
  c =
    c +
    '<li><a class="dropdown-item" href="#" id="blankdocument">Blank Document</a></li>';
  c =
    c +
    '<li><a class="dropdown-item" href="#" id="templates">Templates</a></li>';
  c =
    c + '<li><a class="dropdown-item" href="#" id="createnew">Upload</a></li>';
  c = c + "</ul>";
  c = c + "</div>";
  c = c + "</center></div>";
  ...
  c =
    c +
    '<a class="nav-link" id="drafts"  href="#"><i class="bi bi-journal"></i>  Drafts</a>';
  c = c + "</li>";
  c = c + '<li class="nav-item">';
  c =
    c +
    '<a class="nav-link" id="waiting" href="#"><i class="bi bi-journal-text"></i> Waiting</a>';
  c = c + "</li>";
  //c=c+'<li class="nav-item">';
  ...
  c =
    c +
    '<a class="nav-link" href="#" id="approved"><i class="bi bi-journal-check"></i> Approved</a>';
  c = c + "</li>";
  c = c + '<li class="nav-item">';
  c =
    c +
    '<a class="nav-link" id="rejected" href="#"><i class="bi bi-journal-x"></i> Rejected</a>';
  c = c + "</li>";
  c = c + "</ul>";
  c = c + "</div></div>";
  // fetch the documents
  ...
  c =
    c +
    "<tr><td>#</td><td>Description</td><td>File Name</td><td>Size</td><td>Last Update</td><td>Status</td></tr>";
  for (let i = 0; i < documentsJSON.length; i++) {
    ...
    c =
      ...
      documentsJSON[i].id +
      '">' +
      documentsJSON[i].originalfilename +
      "</td>";
  ...
  if (documentsJSON.length <= 10) {
    ...
    c =
      c +
      '<img src="img/blank-document.svg" class="card-img-top" alt="Blank Document" height="50" style="Padding:2px">';
    c = c + '<div class="card-body">';
    c = c + '<h5 class="card-title">Blank Document</h5>';
    c =
      c +
      '<p class="card-text">You can create a blank document and use the inline block editor to create your agreement. The system keep a version of the different drafts till the signature.</p>';
    c =
      c +
      '<button type="button" class="btn btn-primary" id="create-blank">Create</button>';
    c = c + "</div>";
    c = c + "</div>";
    c = c + "<p></p>";
    c = c + "</div>";
    ...
    c =
      c +
      '<img src="img/templates-document.svg" class="card-img-top" alt="Templates" height="50" style="Padding:2px">';
    c = c + '<div class="card-body">';
    c = c + '<h5 class="card-title">Templates</h5>';
    c =
      c +
      '<p class="card-text">You can create a document from one of the public template, create yours private template and reuse and even make a public one an be rewarded for your contribution.</p>';
    c =
      c +
      '<button type="button" class="btn btn-primary" id="select-template">Select</button>';
    c = c + "</div>";
    c = c + "</div>";
    c = c + "<p></p>";
    c = c + "</div>";
    ...
    c =
      c +
      '<img src="img/upload-document.svg" class="card-img-top" alt="Upload"  height="50" style="Padding:2px">';
    c = c + '<div class="card-body">';
    c = c + '<h5 class="card-title">Upload</h5>';
    c =
      c +
      '<p class="card-text">You can upload the document to be signed, the supported format are: pdf, png, jpeg, doc, docx (Microsoft Word),odf,odc (Open Office).Preview is supported.</p>';
    c =
      c +
      '<button type="button" class="btn btn-primary" id="upload-document">Upload</button>';
    c = c + "</div>";
    c = c + "</div>";
    c = c + "<p></p>";
    c = c + "</div>";
  ...
  if (documentsJSON.length == 0) {
    document.getElementById("upload-document").onclick = () => {
      render_file_upload();
    };
    // connect the button to the editing of an empty document
...
function uploadFile(file) {
  ...
  fetch(url, {
    ...
    .then(async (answer) => {
      ...
      //check answer
      if (
        answerJSON.answer == "KO" &&
        answerJSON.message == "Token is not valid"
      ) {
        logout();
        return;
      }
...
function render_file_upload() {
  ...
  c =
    c +
    '<div class="col-12 pb-1" style="background-color:#D2E3FF" ><center><h2>Upload Documents</h2></center></div>';
  c = c + "</div>";
  ...
  c =
    c +
    '<input type="file" id="fileElem" name="fileElem" accept=".doc,.docx,.pdf,.rtf,.odt,.ods,.odf,.txt" multiple>';
  c =
    c + '<label class="text" for="fileElem"> Drag and drop files here</label>';
  c = c + "</form>";
  c = c + "</div>";
  c = c + "</div>";
  c = c + "</div>";
...
function render_editor_document(docdata) {
  ...
  c =
    c +
    '<div class="col-12 pb-1" style="background-color:#D2E3FF" ><center><h2>';
  if (typeof docdata === "undefined") c = c + "New Document";
  else c = c + "Editing Document";
  c = c + "</h2></center>";
  //button to save an
  ...
  let documentdata = {};
  if (typeof docdata !== "undefined") documentdata = docdata;
...
async function render_templates(tagfilterv) {
  ...
  let url =
    window.location.protocol + "//" + window.location.host + "/templatestags";
  ...
  c =
    c +
    '<select class="form-select" size="10" aria-label="size 10 select templates" id="templateslist">';
  x = templates.length;
  for (i = 0; i < x; i++) {
  ...
  c =
    c +
    '<center><br><button class="btn btn-primary" id="templateclone">Clone</button> ';
  c =
    c +
  ...
  for (i = 0; i < x; i++) {
    ...
    const v =
      tagname +
      ".addEventListener('click',tagfilter);" +
      tagname +
      '.param="' +
      tags[i] +
      '";';
...
async function showtemplate() {
  ...
  const url =
    window.location.protocol + "//" + window.location.host + "/templateview";
  let response = await fetch(url + `?${qs.stringify(params)}`, {
    method: "GET",
  });
...
async function render_settings() {
  ...
  c =
    c +
    '<div class="col-12 pb-1" style="background-color:#D2E3FF" ><center><h2>DocSig - Settings</h2></center></div>';
  c = c + '<div id="msg"></div>';
  ...
  c =
    c +
    '<a class="nav-link active" id="standardSignature"  href="#"><i class="bi bi-vector-pen"></i> Signature</a>';
  c = c + "</li>";
  c = c + '<li class="nav-item">';
  c =
    c +
    '<a class="nav-link" id="Encryption" href="#"><i class="bi bi-file-earmark-lock2"></i> Encryption</a>';
  c = c + "</li>";
  c = c + "</ul>";
  c = c + "</div></div>";
  ...
  c =
    c +
    '<input class="form-control" type="text" placeholder="" aria-label="default input fullname" id="fullname" required >';
  c = c + "</div>";
  c = c + '<div class="col-1">';
  c = c + '<label for="initials" class="form-label">Initials</label>';
  c =
    c +
    '<input class="form-control" type="text" placeholder="" aria-label="default input initials" id="initials" required>';
  c = c + "</div></div>";
  c = c + '<div class="row"><div class="col-1"></div>';
  c = c + '<div class="col-6">';
  // buttons
  c =
    c +
    '<br><button type="button" class="btn btn-primary" id="saveButton">Save</button>';
  c =
    c +
  ...
  c =
    c +
    '<div class="row"><div class="col-1"></div><div class="col-9"><hr></div></div>';
  ...
  // fetch data
  let url =
    window.location.protocol + "//" + window.location.host + "/signaturefonts";
  const response = await fetch(url + `?${qs.stringify(params)}`, {
    method: "GET",
  });
  ...
  c =
    c +
    '<input class="form-check-input" type="radio" name="signatureradio" id="signatureopt" value="fileupload">';
  c = c + "</div>";
  c = c + "</div>";
  c = c + '<div class="col-6 bg-light">';
  c =
    c +
    '<input class="form-control" type="file" accept="image/*" id="signaturefile">';
  let paramss = {
    account: currentAccount.address,
    token: currentToken,
    type: "S",
  ...
  c =
    ...
    '<img src="' +
    urls +
    '" id="previewSignature" style="max-height:100px;max-width:300px;height:auto;">';
  c = c + "</div>";
  c = c + '<div class="col-2 bg-light">';
  c =
    c +
    '<input class="form-control" type="file" accept="image/*" id="initialsfile">';
  paramss.type = "I";
  urls =
    window.location.protocol +
    "//" +
  ...
  c =
    ...
    '<img src="' +
    urls +
    '" id="previewInitials" style="max-height:100px;max-width:200px;height:auto;">';
  c = c + "</div></div>";
  let background = "bg-white";
  for (let i = 0; i < fonts.length; i++) {
    if (fonts[i].length == 0) continue;
    c = c + '<div class="row">';
    c = c + '<div class="col-1"></div>';
    ...
    c =
      c +
      '<input class="form-check-input" type="radio" name="signatureradio" id="signatureopt' +
      i +
      '" value="' +
      i +
      '">';
    ...
    c = c + "</div></div>";
    nrFonts = nrFonts + 1;
    if (background == "bg-light") background = "bg-white";
    else background = "bg-light";
  }
  c =
    c +
    '<div class="row"><div class="col-1"></div><div class="col-9"><hr></div></div>';
  ...
  url =
    window.location.protocol + "//" + window.location.host + "/getsignature";
  url = url + `?${qs.stringify(params)}`;
  ...
  if (typeof answerJSON.fullname == "undefined") {
    ...
    if (answerJSON.fullname == "")
      document.getElementById("fullname").value = "John Doe";
    else document.getElementById("fullname").value = answerJSON.fullname;
    if (answerJSON.initials == "")
      document.getElementById("initials").value = "JD";
    else document.getElementById("initials").value = answerJSON.initials;
    // set the fontname
...
async function render_encryption() {
  ...
  c =
    c +
    '<a class="nav-link" id="standardSignature"  href="#"><i class="bi bi-vector-pen"></i> Signature</a>';
  c = c + "</li>";
  c = c + '<li class="nav-item">';
  c =
    c +
    '<a class="nav-link active" id="Encryption" href="#"><i class="bi bi-file-earmark-lock2"></i> Encryption</a>';
  c = c + "</li>";
  c = c + "</ul>";
  c = c + "</div></div>";
  c = c + '<div class="row"><div class="col-1"></div><div class="col-9">';
  ...
  if (aj.answer == "KO" || aj.encryptionkey == "") {
    ...
    c =
      c +
      '<p class="card-text">To encrypt end-to-end the documents, we have to use a keys pair (elliplict curve Ed22519). A random mnemonic phrase has been generated in your browser session. Your private key is derived from the mnemonic phrase.</p>';
    c =
      c +
      '<p class="card-text">You can use the mnemonic phrase to recover the encryption password in case of lost.</p>';
    c =
      c +
      '<p class="card-text">Please store it safely on paper or other non-electronic medium.</p>';
    c =
      c +
      '<p class="card-text"><div class="alert alert-warning" role="alert">We cannot recover your documents if you loose the password and the mnemonic phrase.</div></p>';
    c =
      c +
      '<label for="mnemonicPhrase" class="form-label">Mnemonic Phrase</label>';
    c = c + '<div class="input-group mb-3">';
    c =
      c +
      '<input class="form-control" type="text" placeholder="" aria-label="default input mnemonicPhrase" id="mnemonicPhrase" value="' +
      mnemonic +
      '" disabled>';
    ...
    c =
      c +
      '<input class="form-control" type="password" placeholder="" aria-label="default input password" id="password" required >';
    c = c + '<i id="strengthPassword" class="badge displayBadge">Weak</i>';
    c =
      c +
      "<p><small>Minimum 8 chars, upper and lower case letters,numbers and one symbol</small></p>";
    c =
      c +
      '<label for="repeatpassword" class="form-label">Repeat Password</label>';
    c =
      c +
      '<input class="form-control" type="password" placeholder="" aria-label="default input repeatpassword" id="repeatpassword" required >';
    ...
    c =
      c +
      '<div class="row"><div class="col-1"></div><div class="col-9" id="msgbottom"></div></div>';
    c =
      c +
      '<br><button type="button" class="btn btn-primary" id="saveButton">Save</button>';
    c =
    ...
    c =
      c +
      '<p class="card-text">The encryption keys have been already created and safely stored. You can change their password:</p>';
    c = c + '<label for="oldpassword" class="form-label">Old Password</label>';
    c =
      c +
      '<input class="form-control" type="password" placeholder="" aria-label="default input oldpassword" id="oldpassword" required >';
...
async function save_encryption() {
  ...
  let url =
    window.location.protocol + "//" + window.location.host + "/saveprivatekey";
  url = url + `?${qs.stringify(params)}`;
  const response = await fetch(url, { method: "GET" });
  let answerJSON = await response.json();
...
async function render_signatures() {
  ...
  for (let i = 0; i < fonts.length; i++) {
    ...
    if (fontsFlag == false) {
      const fontFile = new FontFace("font" + i, "url(" + fonts[i] + ")");
...
async function save_signature() {
  ...
  let url =
    window.location.protocol + "//" + window.location.host + "/updatesignature";
  url = url + `?${qs.stringify(params)}`;
  const response = await fetch(url, { method: "GET" });
  let answerJSON = await response.json();
  // check the answer from the server
...
async function preview_signature() {
  ...
  if (signaturefile.length == 0) return;
  ...
  if (extension != ".png" && extension != ".jpg" && extension != ".jpeg") {
    let msg = '<div class="alert alert-danger" role="alert"><center>';
    msg =
      msg + "Wrong file format, you can upload only .png or .jpg/.jpeg files";
    msg = msg + "</center></div>";
    document.getElementById("msg").innerHTML = msg;
    return;
  }
...
async function preview_initials() {
  ...
  // check if the file is selected
  if (initialsfile.length == 0) return;
  // check for file extension
...
async function upload_image_signature() {
  let signaturefile = document.getElementById("signaturefile");
  // check if the file is selected
  if (signaturefile.files.length == 0) return;
  // check for file extension
  const fn = signaturefile.files[0].name;
  const extension = fn.slice(-4).toLowerCase();
  if (extension != ".png" && extension != ".jpg" && extension != ".jpeg") {
  ...
  // upload of the file
  let url =
    window.location.protocol + "//" + window.location.host + "/uploadsignature";
  let formData = new FormData();
  formData.append("account", currentAccount.address);
  formData.append("token", currentToken);
...
async function upload_image_initials() {
  let initialsfile = document.getElementById("initialsfile");
  // check if the file is selected
  if (initialsfile.files.length == 0) return;
  // check for file extension
  const fn = initialsfile.files[0].name;
  const extension = fn.slice(-4).toLowerCase();
  if (extension != ".png" && extension != ".jpg" && extension != ".jpeg") {
  ...
  // upload of the file
  let url =
    window.location.protocol + "//" + window.location.host + "/uploadinitials";
  let formData = new FormData();
  formData.append("account", currentAccount.address);
  formData.append("token", currentToken);
...
async function encrypt_asymmetric_stream(
  msg,
  senderprivatekey,
  senderpublickey,
  recipientpublickeys,
  ...
  for (let i = 0; i < x; i++) {
    ...
    // encrypts by x25519
    const encsecretkey = sodium.crypto_box_easy(
      secretkey,
      nonce,
  ...
  // import the key
  const tmpkeyaes = await window.crypto.subtle.importKey(
    "raw",
    secretkeyaes,
    alg,
    false,
...
async function decrypt_asymmetric_stream(encmsgb, privatekey, publickey) {
  ...
  if (
    typeof encsecret25519 === "undefined" ||
    typeof nonce25519 === "undefined"
  ) {
    return false;
  }
  ...
  let secretkey;
  try {
    secretkey = sodium.crypto_box_open_easy(
      encsecret25519,
      nonce25519,
...
async function decrypt_symmetric_stream(encmsgb, password) {
  ...
  let tmpkeyaes;
  try {
    tmpkeyaes = await window.crypto.subtle.importKey(
      "raw",
      secretkeyaes,
      alg,
      false,
